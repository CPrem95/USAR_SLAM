...
# from sar_slam.src import plot_funcs as plot
...
class SAR(Node):
    def __init__(self, topic_name_1, topic_name_2, topic_name_3, topic_name_4):
        ...
        self.sar_area_x = 6500 # SAR area in mm # 4500
        self.sar_area_y = 2500*2 # SAR area in mm

        self.sar_orig_x = 0 # SAR origin in mm: 3000/res = 600
        self.sar_orig_y = 2500 # SAR origin in mm: 2500/res = 500
    ...
    def update_figure(self, pipe):
        ...
        while True:
            if pipe.poll():
                ...
                # keypoints, descriptors = sift.detectAndCompute(positive_sar_blur, None)
                # img_with_keypoints = cv2.drawKeypoints(positive_sar_blur, keypoints, None, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
                
                # # Display the image with keypoints
                # self.sar_ax3.imshow(img_with_keypoints, cmap='jet', animated=True)
        ...
        try:
            while True:
                ...
                if q_size > 0:
                    ...
                    # rc_obs = obs_radar1
                    # self.add_sar2(odom, rc_obs, fov_angs, self.radar_img_dist, self.radar_img, self.pixels_x, self.pixels_y, self.res)
                    ...
                    # execute_task_parallel2(img_dist_x, img_dist_y, odom, obs_radar, img, res, samp_size, half_fov, sar_orig_x, sar_end_y)
                    # execute_task_parallel2(self.img_dist_X, self.img_dist_Y, odom, rc_obs, self.radar_img, self.res, self.samp_size, self.half_fov, self.sar_orig_x, self.sar_end_y)
                    # self.add_sar4(odom, rc_obs, self.img_dist_X, self.img_dist_Y, self.radar_img, self.res, 2500)
                    print("Radar observations added at odom: ", odom)
                    self.get_logger().info(f"{Colors.BLUE}remaining qsize: {q_size -1}{Colors.RESET}")

                    # if step_id % 2 == 0: # Update the figure every 1 step
                    #     if hasattr(self, 'radar1_img_obj'):
                    #         abs_sar = np.abs(self.radar_img)
                    #         self.radar1_img_obj.set_data(abs_sar)
                    #         self.radar1_img_obj.set_clim(vmin=np.min(abs_sar), vmax=np.max(abs_sar))
                    #     else:
                    #         # If image hasn't been initialized, use imshow
                    #         self.radar1_img_obj = self.sar_ax.imshow(np.abs(self.radar_img), cmap='jet', animated=True)
                    #     self.sar_fig.canvas.draw()
                    #     self.sar_fig.canvas.flush_events()
                    if step_id % 2 == 0: # Update the figure every 1 step
                    ...
                    if odom[0] > 100000:
                        savemat('sar_data0.mat', {'obs_radar_1': self.mem_obs_radar_1, 'obs_radar_2': self.mem_obs_radar_2, 'obs_radar_3': self.mem_obs_radar_3, 'obs_radar_4': self.mem_obs_radar_4, 'odom': self.mem_odom, 'img': self.radar_img})
    ...
    '''
    IMORTANTTTTTTTTTTTTTTTTT
    The transformation between the robot frame to the radar has to be considered!!!!!!!!
    '''
    # Extract pixels using brute force method [NOT RECOMMENDED]
    def add_sar(self, odom, obs_radar, img_dist, img, pixels_x, pixels_y, res):
        # print('obs_radar: ', obs_radar)
        start_time = time.time()  # Start the timer
        for i in range(0, pixels_y):
            for j in range(0, pixels_x):
                pix_x = img_dist[0, i, j]
                pix_y = img_dist[1, i, j]

                # Calculate the angle from the radar to the pixel
                alpha = math.atan2(pix_y - odom[1], pix_x - odom[0]) - odom[2] 
                if (np.pi/3 < alpha < 2*np.pi/3):
                    # Calculate the distance from the radar to the pixel
                    d_sq = (pix_x - odom[0])**2 + (pix_y - odom[1])**2
                    if (16e4 < d_sq < 25e6): # 400^2 < d < 5000^2
                        # near_ind = self.find_nearest_index2(self.dists, d)
                        # img[i, j] += obs_radar[near_ind]
                        val = self.find_nearest_val(obs_radar, np.sqrt(d_sq), self.samp_size)
                        img[i, j] += val
    ...
    # Extract pixels using brute force method [NOT RECOMMENDED]
    def add_sar2(self, odom, obs_radar, fov_angs, img_dist, img, pixels_x, pixels_y, res):
        # print('obs_radar: ', obs_radar)
        start_time = time.time()  # Start the timer
        count = 0
        for i in range(0, pixels_y):
            for j in range(0, pixels_x):
                ...
                # Calculate the angle from the radar to the pixel
                alpha = math.atan2(pix_y - odom[1], pix_x - odom[0]) - odom[2]
                if (fov_angs[0] < alpha < fov_angs[1]):
                    # Calculate the distance from the radar to the pixel
                    d_sq = (pix_x - odom[0])**2 + (pix_y - odom[1])**2
                    if (16e4 < d_sq < 25e6): # 400^2 < d < 5000^2
                        # near_ind = self.find_nearest_index2(self.dists, d)
        ...
        print('Number of pixels added: ', count)
        print('Time taken to add SAR: ', time.time() - start_time)
        return img 
    ...
    def add_sar_radar_1(self, odom, obs_radar, img_dist_x, img_dist_y, img, res):
        ...
        # val = np.array([self.find_nearest_val(obs_radar, d, self.samp_size) for d in d_sqr])
    ...
    def add_sar_radar_1(self, odom, obs_radar, img_dist_x, img_dist_y, img, res):
        ...
        pix_count = len(pixels[0])
        img_cpy = img.copy()
        # results = Parallel(n_jobs=-1)(delayed(update_img)(id, pixels, img_dist_x, img_dist_y, odom, obs_radar, img_cpy, self.samp_size) for id in range(pix_count)) # id, pixels, img_dist_x, img_dist_y, odom, obs_radar, img, samp_size
        for id in range(pix_count):
            p_y = pixels[0][id]
            p_x = pixels[1][id]
            ...
            # d_sq = (pix_x - odom[0])**2 + (pix_y - odom[1])**2
            d_sqr = np.hypot(pix_x - sensor_base[0], pix_y - sensor_base[1])
            val = self.find_nearest_val(obs_radar, d_sqr, self.samp_size)
            img[p_y, p_x] += val
    ...
    '''
    def add_sar_radar_0(self, odom, obs_radar, img_dist_x, img_dist_y, img, res):
        start_time = time.time()
        pixels, sensor_base = sar.extract_pixels_radar_2(img, res, odom, 400, 3000, self.half_fov, img_dist_x, img_dist_y, self.sar_orig_x, self.sar_end_y, self.radar_r)
        print('Time taken to extract pixels: ', time.time() - start_time)
        pix_count = len(pixels[0])
        ...
        return img
    '''
    # Extract pixels using Bresenham method
    def add_sar4(self, odom, obs_radar, img_dist_x, img_dist_y, img, res, radar_range):
        start_time = time.time()
        cone_th_1 = odom[2] + math.pi/2 - self.half_fov
        cone_th_2 = odom[2] + math.pi/2 + self.half_fov
        pt_l = [odom[0] + radar_range*math.cos(cone_th_1), odom[1] + radar_range*math.sin(cone_th_1)]
        pt_r = [odom[0] + radar_range*math.cos(cone_th_2), odom[1] + radar_range*math.sin(cone_th_2)]

        odom_ind = sar.cart2index(odom[0], odom[1], self.sar_orig_x, self.sar_end_y, self.res)
        pt_l_ind = sar.cart2index(pt_l[0], pt_l[1], self.sar_orig_x, self.sar_end_y, self.res)
        pt_r_ind = sar.cart2index(pt_r[0], pt_r[1], self.sar_orig_x, self.sar_end_y, self.res)
    ...
    '''
    Parallel processing for the SAR image generation
    def process_pixel(id, pixels, img_dist_x, img_dist_y, odom, obs_radar, samp_size):
        p_y = pixels[0][id]
        p_x = pixels[1][id]
        pix_x = img_dist_x[0, p_x]
        pix_y = img_dist_y[p_y, 0]
        d_sq = (pix_x - odom[0])**2 + (pix_y - odom[1])**2
        val = self.find_nearest_val(obs_radar, np.sqrt(d_sq), samp_size)
        return (p_y, p_x, val)
    def parallel_extract_pixels(self, img, res, odom, obs_radar):
        start_time = time.time()
        pixels = sar.extract_pixels(img, res, odom, 400, 3000, self.half_fov, self.img_dist_X, self.img_dist_Y, self.sar_orig_x, self.sar_end_y)
        print('Time taken to extract pixels: ', time.time() - start_time)
        pix_count = len(pixels[0])

        with ProcessPoolExecutor(max_workers=4) as executor:
            results = list(executor.map(lambda id: process_pixel(id, pixels, self.img_dist_X, self.img_dist_Y, odom, obs_radar, self.samp_size), range(pix_count)))

        for p_y, p_x, val in results:
            img[p_y, p_x] += val
    ...
    def find_nearest_val(self, val_lst, x, delta_x):
        ind = int(x // delta_x)
        rem = x % delta_x
        val1 = val_lst[ind]
        val2 = val_lst[ind + 1]
